...
def fmt_dialogue(dlg):
    ...
    for x in dlg or []:
        ...
        if x.get("halo") not in (None, "", 0, 0.0):
            extra.append("halo=" + ("%g" % float(x["halo"])))
        if x.get("halo_color"):
            extra.append("halo_color=" + str(x["halo_color"]))
        if k in ("caption", "sfx"):
...
def build_from_outline(project, outline, chapter_name="Story"):
    ...
    for i, pd in enumerate(outline.get("pages") or []):
        ...
        # canonical_layout traduit les anciens noms ('2-up' -> '2-rows'): on
        # n'ecrit QUE des noms canoniques dans le project.json.
